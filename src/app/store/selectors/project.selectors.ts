...
export const selectAllActiveProjects = createSelector(selectAllProjects, timers => timers.filter(p => !!p.isActive));
export const selectProject = createSelector(selectAllProjects, (timers, props) => timers.find(p => p.id === props.id));

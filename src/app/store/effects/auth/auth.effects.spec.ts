...
describe('login$', () => {
  ...
  it('dispatches login errors', (done) => {
    const authenticationService = TestBed.inject(AuthenticationService);
    (authenticationService.login as jest.Mock).mockRejectedValue(new Error('The login failed'));
    actions$ = of(login({ email: 'test@testty.com', password: 'mysecret' }));
    effects.login$.subscribe((action) => {
      expect(action).toEqual(loginFailure({ error: new Error('The login failed') }));
      done();
    });
  });
...
describe('logout$', () => {
  ...
  it('dispatches login errors', (done) => {
    const authenticationService = TestBed.inject(AuthenticationService);
    (authenticationService.logout as jest.Mock).mockRejectedValue(new Error('The logout failed'));
    actions$ = of(logout());
    effects.logout$.subscribe((action) => {
      expect(action).toEqual(logoutFailure({ error: new Error('The logout failed') }));
      done();
    });
  });
...
describe('resetPassword$', () => {
  ...
  it('dispatches errors', (done) => {
    const authenticationService = TestBed.inject(AuthenticationService);
    (authenticationService.sendPasswordResetEmail as jest.Mock).mockRejectedValue(new Error('The reset failed'));
    actions$ = of(resetPassword({ email: 'test@testty.com' }));
    effects.resetPassword$.subscribe((action) => {
      expect(action).toEqual(resetPasswordFailure({ error: new Error('The reset failed') }));
      done();
    });
  });
